...
mirror_manifest_url="https://whua.me/protolink-release/releases/latest/protolink-latest.json"
tmpdir="$(mktemp -d)"
download_first() {
  output="$1"
  shift
  for url in "$@"; do
    if curl --http1.1 --retry 2 --connect-timeout 10 --max-time 120 -fsSL "$url" -o "$output"; then
      return 0
    fi
  done
  return 1
}
download_first "$tmpdir/protolink-latest.json" "$manifest_url" "$mirror_manifest_url"
node -e "const fs=require('fs'); const m=JSON.parse(fs.readFileSync(process.argv[1],'utf8')); for (const k of ['version','tarballUrl','tarballSha256','cliEntrySha256']) { if (!m[k]) throw new Error('missing '+k); } const urls=[m.tarballUrl,...(m.tarballUrls||[])].filter(Boolean); console.log(urls.join('\n'))" "$tmpdir/protolink-latest.json" > "$tmpdir/tarball-urls"
downloaded_tarball=0
while IFS= read -r tarball_url; do
  if [ -n "$tarball_url" ] && curl --http1.1 --retry 2 --connect-timeout 10 --max-time 120 -fL "$tarball_url" -o "$tmpdir/protolink.tgz"; then
    downloaded_tarball=1
    break
  fi
done < "$tmpdir/tarball-urls"
test "$downloaded_tarball" = "1"
...
- `inspect-skills` / `update-skills` 默认检查并同步 Claude、TRAE、TRAE CN、Codex、Cursor，同时自动发现已安装 bootstrap `protolink` Skill 的其他 Agent skills 目录并同步三件套；`--include-claude` 保留为兼容参数。
